...
### Anti-generic-AI detector (vault tool)

Run the bundled "impeccable detector" to mechanically catch generic-AI tell-tales
(overused fonts like Inter/Space Grotesk, buzzword microcopy, dark/colored glow shadows,
saturated purple→blue gradients, stock layouts) that reviewers often miss:

```bash
node _Config/.opencode/scripts/impeccable-detector/run.mjs <ui-dir-or-file> [--json] [--scope type,layout,...]
```

- Pure-Node subset of `pbakaus/impeccable` (Apache-2.0), no external dependencies
  (degrades to regex mode when the HTML parser packages aren't installed — still catches the
  high-value tells).
- Exit 0 = clean; exit 2 = primary findings. `--scope` filters to design domains
  (`type`, `layout`, ...); `--json` emits machine-readable findings.
- Fix findings in one pass, then re-run once to confirm — don't loop on open-ended self-QA.
